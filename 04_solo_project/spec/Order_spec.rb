...
describe Order do
  ...
  describe '#add' do 
    it 'adds an item to the order array' do 
      my_order = Order.new
      menu = double(:menu)
      allow(menu).to receive(:menu).and_return([{ name: 'burger', price: 4.50, quantity: 10 }, { name: 'hot dog', price: 3.00, quantity: 10 }, { name: 'CocaCola', price: 1.00, quantity: 5 }, { name: 'chips', price: 2.00, quantity: 1 }])
    ...
    it 'fails when adding something that is not on the menu' do 
      my_order = Order.new
      menu = double(:menu)
      expect(menu).to receive(:menu).and_return([{ name: 'burger', price: 4.50, quantity: 10 }, { name: 'hot dog', price: 3.00, quantity: 10 }, { name: 'CocaCola', price: 1.00, quantity: 5 }, { name: 'chips', price: 2.00, quantity: 1 }])
    ...
    it 'fails when adding something that is out of stock' do 
      my_order = Order.new
      menu = double(:menu)
      allow(menu).to receive(:menu).and_return([{ name: 'burger', price: 4.50, quantity: 10 }, { name: 'hot dog', price: 3.00, quantity: 10 }, { name: 'CocaCola', price: 1.00, quantity: 5 }, { name: 'chips', price: 2.00, quantity: 0 }])
